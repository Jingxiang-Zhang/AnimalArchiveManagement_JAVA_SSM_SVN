		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header alert alert-success">
					<h4 class="modal-title" id="myModalLabel">档案推广</h4>
...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page(pn) {
		...
		$.ajax({
		url : "${root_path}advert/advert/query",
			data : "pn=" + pn,
			type : "GET",
	...
	function build_emps_table(result) {
			...
			$.each(emps, function(index, item) { 
				...
				var editbtn = $("<button></button>").addClass(
						...
						$("<span></span>").addClass(
								"glyphicon glyphicon-pencil")).append("推广");
...
<script type="text/javascript">
		...
		$("#emp_update_btn").click(function() {
			...
			$.ajax({
				url : "${root_path}advert/advert/updatearchive/" + $(this).attr("edit-id"),
				type : "PUT",
				data : "archivenumber=" + $(this).attr("edit-id") + 
						"&advertsstate=" + advertstate,
				success : function(result) {
					if (result.code == 100) {
						$.ajax({
							url : "${root_path}advert/advert/insertadvertprotocol",
							type : "POST",
...
//拒绝按钮脚本
	...
	//拒绝
		...
		$("#confirm_del_btn").click(function(){
			...
			//确定，发送拒绝请求
			$.ajax({
				url : "${root_path}advert/advert/updatearchive/" + $(this).attr("delete-id"),
				type : "PUT",
