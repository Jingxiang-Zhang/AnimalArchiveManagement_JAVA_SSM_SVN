		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header alert alert-success">
					<h4 class="modal-title" id="myModalLabel">动物饲养记录修改</h4>
				...
				<div class="modal-body" style="height:550px;overflow:auto;">
					...
					<!-- 显示所有不可被修改的模块 -->
						<div class="form-group">
							<div class="col-sm-5">
								<p class="form-control-static" id="feedId_update_static"></p>
							</div>
							<div class="col-sm-5">
						...
						<div class="form-group">
						    ...
						    <div class="col-lg-12">
						      	<textarea class="form-control" rows="2" id="feedDescription_update_input">
						      	</textarea>
						    </div>					
						</div>
						...
						<div class="form-group">
						    ...
						    <div class="col-lg-12">
						      	<textarea class="form-control" rows="2" id="feedObject_update_input">
						      	</textarea>
						    </div>					
						</div>	
						<div class="form-group">
						    ...
						    <div class="col-lg-12">
						      	<textarea class="form-control" rows="2" id="feedTimes_update_input">
						      	</textarea>
						    </div>					
						</div>		
					</form>
...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page(pn) {
		...
		$("#loading").show();
		$.ajax({
		url : "${root_path}update/feed_record/query",
			data : "pn=" + pn,
			type : "GET",
	...
	function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var exppur=item.statusbeforefeed;
				if(exppur.length>20){
...
<script type="text/javascript">
	...
	function getBreedInfo(id) {
			...
			var feedinfo = page_information.extend.pageInfo.list;
			...
			$.each(animalinfo, function(index, item) {
				if(feedinfo[index].id==id){
					$("#feedId_update_static").text("繁殖档案编号："+ feedinfo[index].id);
					$("#recorderId_update_static").text("上一个记录员编号："+ feedinfo[index].recorder);
					$("#recordDate_update_static").text("上一次饲养记录日期："+ date[index]);
					$("#feedDescription_update_input").val(feedinfo[index].statusbeforefeed);
					$("#feedObject_update_input").val(feedinfo[index].feedobject);
					$("#feedTimes_update_input").val(feedinfo[index].feedprocess);
...
<script type="text/javascript">
		...
		$("#emp_update_btn").click(function() {
			...
			//保存数据
			$.ajax({
				url : "${root_path}update/feed_record/update/" + $(this).attr("edit-id"),
				type : "PUT",
				data : "id=" + $(this).attr("edit-id") + 
						"&recorder=${user_uid}" + 
						"&statusbeforefeed=" + $("#feedDescription_update_input").val() +
						"&feedobject=" + $("#feedObject_update_input").val() +
						"&feedprocess=" + $("#feedTimes_update_input").val() ,
				success : function(result) {
					if (result.code == 100) {
						//关闭模态框
