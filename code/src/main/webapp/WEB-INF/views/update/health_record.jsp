		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header alert alert-success">
					<h4 class="modal-title" id="myModalLabel">动物健康记录修改</h4>
				...
				<div class="modal-body" style="height:550px;overflow:auto;">
					...
					<!-- 显示所有不可被修改的模块 -->
						<div class="form-group">
							<div class="col-sm-5">
								<p class="form-control-static" id="healthId_update_static"></p>
							</div>
							<div class="col-sm-5">
						...
						<!-- 显示可被修改的输入框 -->
						<div class="form-group">
							<label for="inputGender" name="healthlevel"
								class="col-sm-4 control-label">健康状态</label>
							<div class="col-sm-8">
								<label class="radio-inline"> <input type="radio"
									name="healthlevel" id="healthlevel1_input" value="健康"
									checked="checked"> 健康
								</label> <label class="radio-inline"> <input type="radio"
									name="healthlevel" id="healthlevel2_input" value="生病"> 生病
								</label>
							</div>
						</div>
						<div class="form-group">
						    ...
						    <div class="col-lg-12">
						      	<textarea class="form-control" rows="2" id="healthDescription_update_input">
						      	</textarea>
						    </div>					
						</div>		
					</form>
...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page(pn) {
		...
		$("#loading").show();
		$.ajax({
		url : "${root_path}update/health_record/query",
			data : "pn=" + pn,
			type : "GET",
	...
	function build_emps_table(result) {
			...
			var emps2 = result.extend.pageInfo2.list;
			var dates = result.extend.date;
			$.each(emps, function(index, item) {
				//alert(item.empName);
				var animulnumberTd = $("<td></td>").append(item.basicnumber);
				var animulspeciesTd = $("<td></td>").append(item.animalspecies);
				var recorderTd = $("<td></td>").append(emps2[index].recorder);
				var recorddateTd = $("<td></td>").append(dates[index]);
				var healthstateTd = $("<td></td>").append(emps2[index].healthlevel);
				var exppur=emps2[index].healthdescription;
				if(exppur.length>20){
					exppur=exppur.substring(0,19)+"......";
				}
				var healthdescriptionTd = $("<td></td>").append(exppur);
				...
				$("<tr></tr>").append(animulnumberTd).append(animulspeciesTd).append(
						recorderTd).append(recorddateTd).append(healthstateTd).append(
						healthdescriptionTd).append(btntd).appendTo("#emps_table tbody");
			});
	}
...
<script type="text/javascript">
	...
	$(document).on("click", ".edit_btn", function() {
			//查出动物信息
			getHealthInfo($(this).attr("edit-id"));
			//把动物的ID传递给更新按钮
	...
	function getHealthInfo(id) {
			...
			var animalinfo = page_information.extend.pageInfo.list;
			var healthinfo = page_information.extend.pageInfo2.list;
			var date = page_information.extend.date;
			$.each(animalinfo, function(index, item) {
				if(animalinfo[index].basicnumber==id){
					$("#healthId_update_static").text("档案编号："+ healthinfo[index].archivenumber);
					$("#recorderId_update_static").text("上一个记录员编号："+ healthinfo[index].recorder);
					$("#recordDate_update_static").text("上一次健康记录日期："+ date[index]);
					
					$("#empUpdateModal input[name=healthlevel]").val([healthinfo[index].healthlevel ]);
					$("#healthDescription_update_input").val(healthinfo[index].healthdescription);
...
<script type="text/javascript">
		...
		$("#emp_update_btn").click(function() {
			...
			var healthlevel_input;
			if($("#healthlevel1_input").prop("checked")){
				healthlevel_input="健康";
			}
			else{
				healthlevel_input="生病";
			}
			//保存数据
			$.ajax({
				url : "${root_path}update/health_record/update/" + $(this).attr("edit-id"),
				type : "PUT",
				data : "id=" + $(this).attr("edit-id") + 
						"&recorder=${user_uid}" + 
						"&healthlevel=" + healthlevel_input + 
						"&healthdescription=" + $("#healthDescription_update_input").val(),
				success : function(result) {
					if (result.code == 100) {
						//关闭模态框
