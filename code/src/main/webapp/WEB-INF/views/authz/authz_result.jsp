		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header alert alert-success">
					<h4 class="modal-title" id="myModalLabel">是否取消申请？</h4>
...
<script type="text/javascript">
	...
	"<div class=\"row\">"+
			"<div class=\"col-md-12\">"+
				"<table class=\"table table-striped\" id=\"emps_table2\">"+
					"<thead>"+
...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page(pn) {
		...
		$.ajax({
		url : "${root_path}authz/archive_without_process/"+"${user_uid}",
	...
	function build_emps_table(result) {
			...
			$.each(emps, function(index, item) { 
				...
				var editbtn = $("<button></button>").addClass(
						...
						$("<span></span>").addClass(
								"glyphicon glyphicon-pencil")).append("取消申请");
...
<script type="text/javascript">
	...
	$(document).on("click", ".edit_btn", function() {
			//查出动物信息
			$("#archive_number_static").text("档案编号为 "+ $(this).attr("edit-id"));
			//把动物的ID传递给更新按钮
...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page2(pn) {
		...
		"<div class=\"loader\" style=\"position: relative;\">"+
			"<div class=\"loading-3\">"+
				...
				"<i></i><i></i><i></i><i></i>"+
			"</div>";
		$("#container_loader2").empty();
		$(load).appendTo("#container_loader2");
		$("#loading").show();
		
		$.ajax({
		url : "${root_path}authz/archive_has_authz/"+"${user_uid}",
			data : "pn=" + pn+"&id="+"${user_uid}",
			type : "GET",
			success : function(result) {
				//console.log(result);
	...
	function build_emps_table2(result) {
			...
			$.each(emps, function(index, item) { 
				...
				var belong_people_name =  $("<td></td>").append(item.user.name);
				$("<tr></tr>").append(checkBoxTd).append(archive_number).append(archive_key).append(
						belong_people).append(belong_people_name).appendTo("#emps_table2 tbody");
			});
	}
