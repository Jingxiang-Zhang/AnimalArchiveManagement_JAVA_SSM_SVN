...
//页面加载完成后，发送ajax请求，获取分页数据
	...
	function to_page(pn) {
		...
		$.ajax({
		url : "${root_path}authz/authz/query_aggrement",
			data : "pn=" + pn,
			type : "GET",
	...
	function build_emps_table(result) {
			...
			$.each(emps, function(index, item) { 
				...
				var aggre_id = $("<td></td>").append(item.authozagreeid);
...
<script type="text/javascript">
		...
		function to_page(pn) {
			...
			//根据档案编号查出该档案对应的授权协议
			$.ajax({
				url : "${root_path}authz/authz/query_aggrement_by_archivenumber/" +archivenumber,
				type : "GET",
