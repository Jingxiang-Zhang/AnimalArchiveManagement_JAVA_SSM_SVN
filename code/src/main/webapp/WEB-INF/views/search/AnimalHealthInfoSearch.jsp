			...
			<ul class="nav metismenu" id="side-menu">
				...
				<li class="active"><a href="#"> <i class="glyphicon glyphicon-search"></i>
					...
					<ul class="nav nav-second-level collapse">
						...
						<li class="active"><a href="${root_path}search/AnimalHealthInfoSearch">动物健康记录查询</a>
						</li>
...
<script type="text/javascript">
			...
			<div class="row  border-bottom white-bg dashboard-header"
				id="container_1">
				<h2>实验动物档案管理系统 / 档案查询 / <strong>动物健康记录查询</strong></h2>
			</div>
                        	...
                        	<div class="row">
								...
								<label>健康水平：</label>
									<div class="form-group input-group input-group-sm">
										...
										<input type="text" class="form-control" name="healthlevel"
											placeholder="请输入健康水平...">
									</div>
								</div>
								<div class='col-sm-6'>
								<label>健康描述：</label>
									<div class="form-group input-group input-group-sm">
										...
										<input type="text" class="form-control" name="healthdescription"
											placeholder="请输入健康描述...">
									</div>
								</div>
							</div>
							    ...
							    <label>查询开始日期：</label>
							        <div class="form-group">
							            ...
							            <div class='input-group date input-group input-group-sm' id='datetimepicker1'>
							                <input type='text' class="form-control" name="recorddate" id="r1">
	...
	<script type="text/javascript">
		...
		function to_page(pn) {
			$.ajax({
				url : "${root_path}/search/AnimalHealthInfoSearch/queryHealth",
		...
		function build_emps_table(result) {
			...
			$.each(emps, function(index, item) {
				...
				var nb = $("<td></td>").append(item.archivenumber);
				var rc = $("<td></td>").append(item.recorder);
				var rd = $("<td></td>").append(date);
				var hl = $("<td></td>").append(item.healthlevel);
				var dec = $("<td></td>").append(item.healthdescription);
				if(item.healthlevel=="健康") hea=hea+1;
				if(item.healthlevel=="生病") ill=ill+1;
				$("<tr></tr>").append(id).append(nb).append(rc).append(rd).append(hl).append(dec).appendTo("#emps_table tbody");
			});
